...
# To use it in ipython, I modified some parts.
def hydrating(inputfile:str, outputfolder:str, outputfile: str, keyfile:str,idcolumn:int,mode:str ):
    ...
    with open(output_file_short, 'w') as outfile:
        with open(output_file) as json_data:
            for tweet in json_data:
                ...
                json.dump(t, outfile)
                outfile.write('\n')
#Since I used a data folder to save the data, I need to modify this part
    output_file_noformat = ".\\" + args.outputfolder +"\\" +output_file_noformat+".csv"
    output_file_noformat=''.join(output_file_noformat)
    f = csv.writer(open(output_file_noformat, 'w'))
    print('creating CSV version of minimized json master file') 
    fields = ["favorite_count", "text", "in_reply_to_screen_name", "is_retweet", "created_at", "retweet_count", "id_str"]                
    f.writerow(fields)       
    with open(output_file_short) as master_file:
        for tweet in master_file:
            data = json.loads(tweet)            
            f.writerow([data["favorite_count"], 
                        data["text"].encode('utf-8'), 
                        data["in_reply_to_screen_name"], 
                        data["is_retweet"], 
                        data["created_at"].encode('utf-8'), 
                        data["retweet_count"], 
